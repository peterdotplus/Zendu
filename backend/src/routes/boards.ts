...
boardsRouter.post('/', async (req: AuthedRequest, res) => {
	...
	try {
		const board = await prisma.board.create({ data: { title, description, teamId: teamId ?? null, creatorId } });
...
boardsRouter.get('/', async (req: AuthedRequest, res) => {
	...
	const teamMemberships = await prisma.teamMembership.findMany({ where: { userId }, select: { teamId: true } });
	const teamIds = teamMemberships.map(m => m.teamId);
	const boards = await prisma.board.findMany({
		where: { OR: [{ creatorId: userId }, { teamId: { in: teamIds } }] },
		orderBy: { createdAt: 'desc' },
		select: { id: true, title: true, description: true, teamId: true, createdAt: true, updatedAt: true }
	});
	res.json({ boards });
});
...
boardsRouter.patch('/:boardId', async (req: AuthedRequest, res) => {
	...
	try {
		const board = await prisma.board.update({
			where: { id: boardId },
			data: { title, description, teamId: typeof teamId === 'undefined' ? undefined : teamId }
		});
		res.json({ board });
	} catch (_e) {
...
boardsRouter.post('/:boardId/lists', async (req: AuthedRequest, res) => {
	...
	const max = await prisma.list.aggregate({ _max: { position: true }, where: { boardId } });
	const nextPos = (max._max.position ?? 0) + 1;

	try {
		const list = await prisma.list.create({ data: { boardId, title, position: nextPos } });
...
const reorderListSchema = z.object({ listId: z.string(), toPosition: z.number().int().min(1) });

boardsRouter.patch('/:boardId/lists/reorder', async (req: AuthedRequest, res) => {
	const { boardId } = req.params;
	...
	if (!list || list.boardId !== boardId) return res.status(404).json({ error: 'List not found' });
	...
	await prisma.$transaction(async (tx) => {
		...
		await tx.list.update({ where: { id: listId }, data: { position: toPosition } });
	});
...
// Update list
const updateListSchema = z.object({ title: z.string().min(1).optional(), position: z.number().int().min(1).optional() });
...
boardsRouter.patch('/lists/:listId', async (req: AuthedRequest, res) => {
	...
	try {
		const list = await prisma.list.update({ where: { id: listId }, data: { title, position } });
		res.json({ list });
	} catch (_e) {
...
boardsRouter.post('/lists/:listId/cards', async (req: AuthedRequest, res) => {
	...
	// Compute next position
	const max = await prisma.card.aggregate({ _max: { position: true }, where: { listId } });
	const nextPos = (max._max.position ?? 0) + 1;
...
const reorderCardSchema = z.object({ cardId: z.string(), toPosition: z.number().int().min(1) });

boardsRouter.patch('/lists/:listId/cards/reorder', async (req: AuthedRequest, res) => {
	const { listId } = req.params;
	...
	if (!card || card.listId !== listId) return res.status(404).json({ error: 'Card not found' });
	...
	await prisma.$transaction(async (tx) => {
		...
		await tx.card.update({ where: { id: cardId }, data: { position: toPosition } });
	});
...
const moveCardSchema = z.object({ toListId: z.string(), toPosition: z.number().int().min(1).optional() });
...
boardsRouter.patch('/cards/:cardId/move', async (req: AuthedRequest, res) => {
	...
	await prisma.$transaction(async (tx) => {
		...
		if (!destPos) {
			const max = await tx.card.aggregate({ _max: { position: true }, where: { listId: toListId } });
		...
		// Move the card
		await tx.card.update({ where: { id: cardId }, data: { listId: toListId, position: destPos } });
	});
...
boardsRouter.patch('/lists/:listId/cards/:cardId', async (req: AuthedRequest, res) => {
	const { listId, cardId } = req.params;
	const parse = updateCardSchema.safeParse(req.body);
	if (!parse.success) return res.status(400).json({ error: parse.error.flatten() });
	const data = parse.data as any;
	// Normalize optional nullables
	if (Object.prototype.hasOwnProperty.call(data, 'headerColor') && data.headerColor === '') data.headerColor = null;
	if (Object.prototype.hasOwnProperty.call(data, 'categoryId') && data.categoryId === '') data.categoryId = null;
	try {
...
// Delete card
boardsRouter.delete('/lists/:listId/cards/:cardId', async (req: AuthedRequest, res) => {
	const { cardId } = req.params;
	try {
...
// Categories
const createCategorySchema = z.object({ name: z.string().min(1), colorHex: z.string().regex(/^#?[0-9a-fA-F]{6}$/) });

boardsRouter.get('/:boardId/categories', async (req: AuthedRequest, res) => {
	const { boardId } = req.params;
	const categories = await prisma.category.findMany({ where: { boardId }, orderBy: { name: 'asc' } });
	res.json({ categories });
});
...
boardsRouter.post('/:boardId/categories', async (req: AuthedRequest, res) => {
	...
	try {
		const category = await prisma.category.create({ data: { boardId, name, colorHex } });
		res.status(201).json({ category });
	} catch (_e) {
		res.status(400).json({ error: 'Could not create category (maybe duplicate name)' });
	}
});
...
boardsRouter.get('/lists/:listId/cards/:cardId/comments', async (req: AuthedRequest, res) => {
	const { cardId } = req.params;
	const comments = await prisma.comment.findMany({ where: { cardId }, orderBy: { createdAt: 'asc' } });
	res.json({ comments });
});

boardsRouter.post('/lists/:listId/cards/:cardId/comments', async (req: AuthedRequest, res) => {
	const { cardId } = req.params;
	const parse = commentSchema.safeParse(req.body);
	if (!parse.success) return res.status(400).json({ error: parse.error.flatten() });
	const { content } = parse.data;
	const authorId = req.user!.id;
	const c = await prisma.comment.create({ data: { cardId, authorId, content } });
...
boardsRouter.post('/lists/:listId/cards/:cardId/assignees', async (req: AuthedRequest, res) => {
	const { cardId } = req.params;
	const parse = assigneeSchema.safeParse(req.body);
	if (!parse.success) return res.status(400).json({ error: parse.error.flatten() });
	const { userId } = parse.data;
	try {
		const assignment = await prisma.cardAssignment.create({ data: { cardId, userId } });
		res.status(201).json({ assignment });
	} catch (_e) {
		res.status(400).json({ error: 'Could not assign user (maybe already assigned)' });
	}
});

boardsRouter.delete('/lists/:listId/cards/:cardId/assignees/:userId', async (req: AuthedRequest, res) => {
	const { cardId, userId } = req.params as { cardId: string; userId: string };
	const found = await prisma.cardAssignment.findFirst({ where: { cardId, userId } });
